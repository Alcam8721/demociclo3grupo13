...
def show():
    ...
    # TODO En show.html que se renderice en FROM el nombre de usuario y no el id
    messages = db.execute(
        # 'select * from message where from_id=? or to_id=?', (g.user['id'],g.user['id'])
        'select * from message inner join user on from_id = user.id where from_id=? or to_id=?', (g.user['id'],g.user['id'])
    ).fetchall()
